...
use CZERTAINLY::Configuration;

sub new {
    my $class = shift;

    my $config;
    if ( $_[0] && ref $_[0] && ref $_[0] eq 'CZERTAINLY::Configuration' ) {
        $config = $_[0];
    } else {
        $config = CZERTAINLY::Configuration->new(@_);
    }
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("CZERTAINLY::Object::$class")->new;
        if (ref $data eq "HASH") {
